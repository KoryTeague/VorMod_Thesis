...
\Cref{ch:sim} considered the effectiveness of the two approaches via three separate simulations.  The first simulation was a proof of concept and showed that the two approaches converge to a solution and that the demand model worked as expected.  It showed that the GA approach converged to a solution in much less time than the sDEP, providing a speedup factor of up to 46.3x.  When comparing same cost solutions, the GA introduced only a marginal amount of error---0.3\% less demand satisfaction---for scenarios the sDEP was optimized for and marginally outperformed the sDEP---0.0\%--0.2\% more demand satisfaction---for scenarios the sDEP was not optimized for.  This highlighted the primary limitations of the two approaches.  First, the sDEP, while a direct solution of the original stochastic program, directly trades solution feasibility for accuracy by controlling the number of scenarios and demand points per scenario.  The scenarios in $\hat{\Omega}$ were sufficient to provide a close approximation of the demand field, but the demand satisfaction decreased with more scenarios containing more demand points, showing that a tighter approximation was possible but ignored for feasibility.  Second, the GA targets 100\% demand satisfaction, and was not able to be tuned to other solutions.  This limits the GA in effectiveness, and only allows the GA to be directly compared to the sDEP for similar cost, near 100\% demand satisfaction solutions.

The latter two simulations acted as test cases which utilize more realistic looking parameters.  
