...
\improvement{Mention the relationship between the RPs and the VNB\\See Note}%
%Note: The BSs are made available to be leased to the VNB by the RPs.  The RPs own and maintain the resources, and have provided them to the VNB for aggregation into a common pool of usable resources.  This aggregated pool of resouces forms $\mathcal{S}$ and are available to be used within a constructed set of VWNs for a known (agreed upon by contact/agreement) cost $c_s$.  The resources are currently further defined and detailed by their coverage radius or range ($b_s$), location (?? - I'm tired), and possible capacity ($r_s$).  At this time, the only resources being considered are BSs, so all resources referred to in the remainder of this paper are BSs of some sort.
Consider a geographical area of width $X$ meters and length $Y$ meters that contains a set $\mathcal{S} \defeq \left\{1,\, 2,\, \ldots,\, S\right\}$ of BSs available to be leased to the VNB by a set of $\mathcal{N} \defeq \left\{1,\, 2,\, \ldots,\, N\right\}$ RPs.  The rate capacity of BS $s \in \mathcal{S}$ is denoted by $r_s$, its cost is denoted by $c_s$, and its coverage radius is denoted by $b_s$.
