...
\section{Virtualization, Virtualized Wireless Networks, and the Networks without Borders Paradigm}
...
One approach towards minimizing CAPEX and OPEX of networks has been the utilization of resource sharing.  Resource sharing encompases the sharing of resources between multiple networks, and can take the form of \textit{passive sharing} - referring to the sharing of physical sites, tower masts, cabling, power supplies, and other components that are not actively on part of the network architecture - and \textit{active sharing} - referring to the sharing of the active network architecture itself, such as backhaul and RAN.  The practice has been utilized since 2G and 3G networks as a tool for reducing CAPEX in expanding the network \cite{1421931}.  Since then, resource sharing has become more common, where it is now available, standardized \cite{3GPP_TS_23.251}, and implemented in many major carrier networks.  As reported in \cite{6553675}, a 2010 market survey \cite{NetSharingReport} found that over 65\% of European MNOs have deployed mobile infrastructure sharing in some form.  It was further reported \cite{6553675} that 20\% of cells carry about 50\% of total network traffic, with the other 80\% of cells still causing OPEX without gain.  Through active resource sharing, networks can avoid redundant deployments and wasted capacity, reducing overall CAPEX and OPEX.

\textit{Bridge from resource sharing to virtualization.  Maybe mention how this impacts the value chain for cellular networks.}

%As an extension of this active resource sharing, wireless virtualization is very promising approach for reducing expenditures in next-generation mobile networks.

The increasing prominence of active resource sharing challenges the traditional model of ownership of the various network layers and elements.  Once network operators could utilize resources owned and maintained by other operators, it becomes possible for these MNOs to operate networks primarily or only using these shared resources.  A given shared resource can be decoupled from a specific physical resource, instead enabling it to be adaptively associated with any of a given pool of qualifying physical resources as network conditions allow, establishing the shared resource as a virtual resource.  Further, virtual networks can adapt to changing network conditions, adding and removing virtual resources as capacity requirements change.  For example, a MVNO with a network of purely virtual resources can add additional virtual resources during the peak hour when additional capacity for end user satisfaction is needed, and removing unneeded resources during the night to reduce OPEX of the network.

This concept of virtualization partitions segments the classical wireless networking value chain, allowing for specialization of these subsegments into new entities such as resource providers and service providers \cite{1421931}.  Doyle \textit{et al.} \cite{6737248} investigates the value chain with this segmentation in mind, and introduces the Networks without Borders (NwoB) paradigm as a new market based around constructing VWNs using pools of virtual resources.

%Through efficient sharing of radio resources, wireless virtualization is one the most promising approaches for reducing expenditures in next-generation mobile networks \cite{6824752, 6553675, 6571315}.  With the introduction of virtual resources

\textit{1421931 - Shared Networks: Making Wireless Communication Affordable (2005) - Network sharing has been used as a expenditure saving measure going back to 2G and 3G networks.  It was typically viewed within the industry as a CAPEX saving measure.  However, it also allows for a decomposition of the network value chain ``into network operators, neutral hosts, mobile virtual network operators (MVNOs), service providers, and other entities''.  Allows for increasing specialization of products within the market.  MVNOs are capable of targeting specific classes of users, such as ``students, road warriors, tradespeople, executives, and so on.'' ``Network operation and maintenance and service creation and development are quite obviously very different businesses, and network sharing provides an efficient, manageable means for them to operate with greater independence of each other.''}

\textit{6553675 - Radio Access Network Virtualization for Future Mobile Carrier Networks (2013) - Resource sharing as a concept is a fairly well investigated practice, and is already available and standardized among the major carrier networks.  This can take form as passive and active sharing.  ``Passive sharing refers to the reuse of components such as physical sites, tower masts, cabling, cabinets, power supply, air-conditioning, and so-on.  Active sharing refers to the reuse of backhaul, base stations, and antenna systems, the reuse of the latter two labeles as active RAN sharing.''}
