...
From the perspective of the VNB, each SP is only distinguishable by its overall demand characteristics ().  These demand characteristics are defined by the SPs' demand density distributions\iffalse , on which demand point locations that determine $\tilde{u}_{mns}$ are dependent\fi .  The VNB must construct a VWN for each SP, but for optimal VWNs to be created, the VNB must consider the demands of all SPs simultaneously and in context of each other.  For the VNB, all SP demand points are indistinguishable and independent.  Therefore, the VNB considers a single set of demand points

\begin{equation}
\mathcal{M} \defeq \bigcup_{i=1}^N \mathcal{M}_i = \left\{ 1,\, 2,\, \ldots,\, M \right\},
\end{equation}

\noindent where $M = \sum_{i=1}^N M_i$, with demands $d$, and stochastic normalized capacities $\tilde{u}_{ms},\, m \in \mathcal{M},\, s \in \mathcal{S}$.
...
R\left( dx,\, dy \right) = E\left[ \lambda^G\left( x,\, y \right) \; \lambda^G\left( x + dx,\, y + dy \right) \right] = \frac{1}{4L} \; \sinc\left( \omega_{\max} dx \right) \; \sinc\left( \omega_{\max} dy \right).
\end{equation}
...
Let $\Omega$ be defined as the sample space (i.e., the set of all scenarios) of \improvement{M: I still object to the claim that demand point locations define $u_{ms}$}demand point locations defining $\tilde{u}_{ms}$.  \change{M: Only try for discrete (finite or countably infinite) sample spaces\\Big problem; fix}The probability a given scenario $\omega \in \Omega$ occurs is denoted by $p^{(\omega)}$, where $\sum_{\omega \in \Omega} p^{(\omega)} = 1$.  Variables that are dependent on the scenario are shown with a superscript ($\omega$) with the specific scenario it is dependent on indicated by $\omega$.  By making this change, the DEP (\cref{eq:DEPO,eq:DEPC1,eq:DEPC2,eq:DEPC3,eq:DEPC4}) of the two-stage stochastic optimization program modeled in \Cref{sec:stochopt} is established.
...
\info{M: No comments because of the previous paragraph's major problem.  This all needs some rewriting.}%
%note: because of the previous paragraph's major problem.  This all needs some rewriting.
...
\begin{align}
& \argmin_{\left\{ z_s,\, \delta_{ms}^{\left( \omega \right)} \right\}} \left\{ \sum_{s \in \mathcal{S}} c_s \; z_s - \alpha \sum_{\omega \in \hat{\Omega}} p^{\left( \omega \right)} \left( \sum_{m \in \mathcal{M}} \sum_{s \in \mathcal{S}} \delta_{ms}^{\left( \omega \right)} \; u_{ms}^{\left( \omega \right)} \right) \right\} \label{eq:sDEPO}\\
& \text{subject to:}  \nonumber \\
...
\info{M: Specifically marked this paragraph}%
...
After the solution to the sDEP model has been found, the VNB has determined the joint BS selection that supports the VWNs.  It further provides a proposed resource slicing of the BSs to the SPs' demand points, which determine the VWN constructions for those scenarios considered in $\hat{\Omega}$.  As a condition for the sDEP model, $\hat{\Omega}$ is a finite set that does not comprehensively cover all scenarios.  For an actual implementation of the sDEP, any observed scenario of demand points is certain to not be within $\hat{\Omega}$, and the sDEP solution would not provide a slicing of the selected resources to construct a VWN for the new scenario.

%note: M: I don't think "infeasibility" is the primary problem here.  The problem is one of time scales.  The realizations of demand points happen on a much shorter (faster) time scale than the resource selecitons.
Due to the computational complexity of the sDEP model, it is \info{M: See comment note}infeasible to find a new resource selection as the realization of demand changes.  Further, assuming that the underlying demand characteristics do not change (i.e., that demand realizations still correlate to the demand intensity function, $\lambda_n$), it is unnecessary to determine a new joint resource selection as it is still optimally selected within a given confidence for the demands.  \change{M: Previous two sentences spend a lot of words saying little\\you're overexplaining this.}Instead, it is valuable to have a model which assigns an adaptive slicing of the selected resources so the VWNs can adapt according to specific realizations of demand without needing to determine a new joint resource selection.  This new adaptive slicing model (\cref{eq:ASMO,eq:ASMC1,eq:ASMC2,eq:ASMC3}) is found by fixing the selected resources, $z_s$, and only considering the specific demand realization $\omega \notin \hat{\Omega}$ to be sliced to and satisfied.
...
\change{M: Italics Important}%
\textit{M: I think you need to summarixe the overall approach, either here or earlier in section 3.1.  Namely: sample possible scenarios and solve the sDEP for the sampled distribution of scenarios in order to determine the resource selection.  Then, for realized scenarios, solve the adaptive slicing problem.}

\textit{M: One thing that I'm stuggling with in your writing is that you are delving into unneccessary detail about some fairly obvious things, while failing to provide your reader with a big picture about what is going on.}
...
\mathrm{cost}\left( z^{\left\{ ig \right\}} \right) = \sum_{s \in \mathcal{S}} \Biggl( c_s \; z_s^{\left\{ ig \right\}} + c_\text{cov} \; \ind{V_s \not\subseteq B_s} + \\ \left( c_\text{cap}^g - 1 \right) \; \max\left( 0,\, \sum_{i = 1}^N \left( \iint_{V_s} \lambda_i\left( x,\, y \right)\, dx\, dy \right) - r_s \right) \Biggr)
\end{multline}
...
\improvement{Improve on second halting condition explanation}%
%note: M: I don't understand this second halting condition.
