...
\unsure{Double check all these values and parameters}%
This test case is constructed considering the following considerations and parameters.  There is only one SP, and its demand intensity function $\lambda$ is generated using parameters (i.e., $\omega_{\max},\, \sigma,\, \mu$) that were empirically found to match a real world urban cellular network in China~[?]\change{cite}.  $\lambda$ is pixelated into a 200 by 200 grid of square pixels with 15 m side length; $\mathcal{R}$, illustrated in \Cref{fig:CaseI_NetworkArea}, is a region 3 km by 3 km in size.  Resources are homogeneous and constructed to represent small macrocells.  The parameter values used in the simulations are shown in \Cref{tab:CaseI_SimVal}\improvement{Double check text spacing\\Solo-page if less than 6 lines of text fit on the same page.}.
