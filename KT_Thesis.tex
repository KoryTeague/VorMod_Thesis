...
After the solution to the sDEP model has been found, the VNB has determined the joint BS selection that support the VWNs.  It further provides a proposed resource slicing of the BSs to the SPs' demand points that determine the separate VWN constructions for those scenarios considered in $\hat{\Omega}$.  However, the major limitation of the sDEP model and its solution is its computational complexity; it cannot be run in or close to real time.  As such, 

  As a condition for the sDEP model, $\hat{\Omega}$ is not infinite in scope.  Therefore, any given slicing of the selected BSs
