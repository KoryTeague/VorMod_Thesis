...
The spike of CPU run times corresponds to the regions of $\alpha$ that the sDEP solution is in transition, where the optimal number of resources selected is more difficult for the CPLEX branch-and-cut algorithm to determine.  This can be seen in \Cref{fig:Prelim_sDEPCompCostRunTime}, where the run time spike of $\alpha = \left[ 25,\, 45 \right]$ surrounds the transition between the selection of 9 BSs and 10 BSs.  CPLEX converges to a solution ($\alpha < 25$, 9 BSs; $\alpha > 45$, 10 BSs) within around 1000 seconds CPU time.  However, within this critical region, the CPLEX branch-and-cut algorithm is unable to cut many considered solutions, increasing run time up to the 6000 seconds CPU time.  This trend continues outside of the shown values of $\alpha$; at higher values, such as $\alpha \approx 10^6$, there is another spike where CPLEX is unable to complete within 15 minutes as the solution transitions between 12 and 13 selected BSs.
...
% Then lead into GA Cost for the values tested (Histogram?  Cost vs runtime?).  Talk about the cost characteristics.  Imply that modifying the magnitude of $\lambda$ would be similar to affecting $\alpha$ in the sDEP, and allow the GA to allocate less and more resources fo the VWN

% Directly compare the sDEP and the GA costs

The trade off for the GA's improved run time is that the solution provided is less optimal than the sDEP, as indicated by an increased cost for the VNB to build the VWN.  \Cref{fig:Prelim_VWNCompCost} compares the increasing cost of the sDEP as $\alpha$ increases with the cost of the various GA solutions.  On average, the GA incurs a 36\% increased cost for the VWN, a significant amount of incurred error.

\begin{figure}[ht]
	\centering
	\includegraphics[height=0.45\textheight]{Figures/Prelim_ComparisonCost_VOSGA25ScenarioSet}
	\caption[Preliminary simulation constructed VWN costs]{VWN Costs; 95\% confidence interval shown in black}
	\label{fig:Prelim_VWNCompCost}
\end{figure}

At minimum, the incurred cost is only 20\% than the sampled DEP, which implies the genetic algorithm might be terminating early, and a tighter solution might be found by increasing $G_\text{halt}$.  It should also be noted that one unit of cost is one additional BS being selected for the VWN, and the sampled DEP selections for $\alpha \geq 30$ have a cost of only 10 BSs.  Any variance that incurs one additional BS for the genetic algorithm incurs 10\% increased cost.  Increasing the number of BSs required to comprise the VWN would introduce additional granularity in $\mathcal{S}$ that might decrease the inefficiency of the genetic algorithm.  This was not done as this data set was chosen specifically so the sampled DEP would terminate within 15 minutes (i.e., in a reasonable amount of time); increasing the number of BSs available to the VNB drastically increases the time it would take the sampled DEP to converge to a solution.
