...
In this section I present the results of these preliminary simulations.  \unsure{M: It's a little odd that run time results are your first metric.\\Might flip these to the end.}First, run time results of the two approaches will be presented and comparisons made.  Second, the network cost results will be presented and compared, followed by, third, the network demand satisfaction results.  Finally, this section will end with my conclusions of these simulations
...
\info{See Comment}%
%M: If I understand what you are doing here, then this is weird. Satisfying a demand which is 80% of the original demand is not the same as satisfying 80% of the demand. The latter is more a statement about probability. Or should be.
%M: Namely, if the goal is to satisfy 80% of the demand, then we should be able to choose which 80%. If you just scale the demand, then we lose this choice. 
...
\info{M: To my mind, this discussion of run time is being given too much space and prominence.\\I'm just not sure a detailed analysis of run time is the point.}%
...
Each additional microcell increases the overall demand satisfaction of the constructed network, but fails to fully satisfy without an additional macrocell.  To achieve 100\% demand satisfaction, the network must satisfy 19.97 Gbps\footnote{One scenario includes 90 demand points with each demanding 221.89 Mbps.} of demand, which requires 10 macrocells or 9 macrocells and 20 microcells to satisfy; since there are only 18 microcells available, there are insufficient microcells to bridge the gap between 9 and 10 macrocells\info{M: This is very weird and skews your results.\\See comment on \Cref{tab:CaseII_SimVal}.}.  This is further reflected in \Cref{fig:CaseII_sDEPComparisonSatCaseI}; for $\alpha \in \left[ 10,\, 18 \right]$, the sDEP has approximately 98\% demand satisfaction, and reaches 100\% for $\alpha \geq 20$.
...
\unsure{M: This is an awkward way to end a thesis, but I'm not sure I have a suggestion.\\Ha.}
\fi
\fi
