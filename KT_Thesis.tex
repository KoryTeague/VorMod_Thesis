...
%Note: Consider POV and voice; at least change ``we''s to ``I''s, though that might further change if ``I'' is too personal or unprofessional.
%Note: Ensure equations are spaced well for full page and devoid of unnecessary horizontal or vertical spacing that may have been left behind from the trimming and formatting from the conference paper; this will require a bit of polish later during a formatting pass after most writing is accomplished.
%Note: Improve on overall wording to add consideration for the presence of RPs and SPs in the architecture; BSs come from the RPs to be aggregated into a pool for use by the VNB, and SPs have a service or services to satisfy, and must present that service to the VNB in some coherent manner for the VNB to build an optimal VWN considering other SPs' needs and the available resources.
%Note: BS or Resource?
...
\begin{equation} \label{eq:ums_simp}
u_{mns} \defeq
	\begin{cases}
		0,& \text{if demand point $m$ of SP $n$ is located more than $b_s$ meters from}\\
		& \text{\tab resouce $s$,}\\
...
\end{cases}
\end{equation}
\tab[0.1cm] where $x_i$ and $y_i$ are the x- and y-coordinates of the $i^{\text{th}}$ point of the stationary PPP.  So that the distribution of demand realized by the non-stationary PPP corresponds to the traffic demand density function, the overall demand 
\begin{equation} \label{eq:demandintegral}
D = \int_0^X \int_0^Y \rho\left(x,\, y\right) dy dx
\end{equation}%
\tab[0.1cm] of the demand density distribution is evenly distributed such that
...
\improvement{Consider expanding on these definitions\\autocorrelation functions\\maybe add an image of the resulting $\rho^G$\\Consult SSLT paper for possible details to include}%
...
\tab[0.1cm] where $\mathcal{L} \defeq \left\{1,\, 2,\, \ldots,\, L\right\}$ is a set of the products of two cosines with stochastic angular frequencies $i_l,\, j_l\, \sim \mathcal{U}\left(0,\, \omega_{\max}\right)$\footnote{$\mathcal{U}\left(a,\, b\right)$ refers to a random variable uniformly distributed over the domain $\left[a,\, b\right]$, with minimum value $a$ and maximum value $b$.}$,\, l \in \mathcal{L}$ and phases $\phi_l,\, \psi_l\, \sim \mathcal{U}\left(0,\, 2\pi\right),\, l \in \mathcal{L}$.  As $L$ increases, $\rho^G$ approaches a Gaussian random field according to the central limit theorem.
...
\end{equation}
\tab[0.1cm] The autocorrelation function is notably dependent on the maximum angular frequency defining $\rho^G$, $\omega_{\max}$.  As $\omega_{\max}$ increases, the demand of adjacent regions become less correlated.  $\omega_{\max}$ is effectively a measure of the inhomogeneity of $\rho^G$.
