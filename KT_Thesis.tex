...
\Cref{fig:GARunTime} is 

In \Cref{fig:AlgSpeedupRunTime} is a comparison of the approach run times.  It shows the run times of both the sDEP and GA approaches in terms of the GA speedup ratio and absolute number of seconds.  As the number of scenarios sampled increases, the CPU time of the sDEP increases exponentially, and fails to converge to a final solution with 50 scenarios within the time limit of 15 minutes (wall clock time).  The GA approach simulations converge in 94 to 209 seconds, averaging 125.9 seconds CPU time with 95\% confidence interval $\left( 118.4,\, 135.4 \right)$.

Except for the trivial five scenario case--which allocated zero BSs--the GA approach

Except for the 5 scenario case, the GA approach converges to a solution in less CPU time (mean of 125.8 seconds) than the sDEP.  When 20 scenarios are sampled, the GA provides a 4.4x speedup over the sDEP approach; at 45 scenarios, this increased to 46.3x.  CPLEX is capable of parallelizing across the 8 CPU cores, allowing for the wall clock run time to be, at minimum, one-eighth the CPU run time.  In terms of the adjusted approximate ``wall clock" time, the genetic algorithm converges in less wall clock time than the sampled DEP for 25 or more sampled scenarios.

\begin{figure}[ht]
	\centering
	\includegraphics[height=0.45\textheight]{Figures/VOSGA_SpeedupRatio_AbsComp_alpha20}
	\caption{Run time comparison of the GA and sDEP approaches with $\alpha = 20$}
...
\begin{figure}[ht]
	\centering
	\includegraphics[height=0.45\textheight]{Figures/ComparisonCost}
...
\begin{figure}[ht]
	\centering
	\includegraphics[height=0.45\textheight]{Figures/ComparisonSatisfaction}
...
\begin{figure}[ht]
	\centering
	\includegraphics[height=0.45\textheight]{Figures/ComparisonSatisfactionEval}
