...
\begin{figure}[ht]
	...
	\caption[GA Approach CPU run time PDF over 50 runs of the preliminary simulations]{GA Approach CPU run time PDF over 50 runs.  Mean (125.9 secs) shown in orange, 95\% confidence interval (118.4, 135.4) shown in dashed black.}
...
The spike of CPU run times corresponds to the regions of $\alpha$ that the sDEP solution is in transition, where the optimal number of resources selected is more difficult for the CPLEX branch-and-cut algorithm to determine.  This can be seen in \Cref{fig:Prelim_sDEPCompCostRunTime}, where the run time spike of $\alpha = \left[ 25,\, 45 \right]$ surrounds the transition between the selection of 9 BSs and 10 BSs.  CPLEX converges to a solution ($\alpha < 25$, 9 BSs; $\alpha > 45$, 10 BSs) within approximately 1000 seconds.  However, within this critical region, the CPLEX branch-and-cut algorithm is unable to cut many close-to-feasible solutions, increasing run time up to 6000 seconds.
...
\begin{figure}[ht]
	...
	\caption[Expanded preliminary simulation comparison of sDEP run time and VWN cost]{Comparison of sDEP CPU run time and VWN construction cost for further values of $\alpha$.}
	\label{fig:Prelim_sDEPCompCostRunTimeExpanded}
\end{figure}

The GA approach settles on solutions with four different values of network cost.  \Cref{fig:Prelim_GACostsPMF} visualizes these four types of solutions as a histogram PMF of the 50 GA simulations.  Half of the simulations ended with a selection of 13 BSs.  However, almost half ended with 14 or 15 selected BSs as the optimal selection; this implies that the GA is terminating too early for the solutions to settle into the better, more common 13-BS solution.  This is further implied by the single 12-BS solution, which, like all of the other solutions, is neither overcoverage nor overcapacity.  The 12-BS solution is completely feasible and stronger than all 49 other solutions.
...
\begin{figure}[ht]
	\centering
	\includegraphics[height=0.45\textheight]{Figures/Prelim_GARunTimesVCosts_cis}
...
\begin{figure}[ht]
	\centering
	\includegraphics[height=0.45\textheight]{Figures/Prelim_O25sDEPGACostComparison_Expanded2}
...
The later test cases (\Cref{sec:caseI,sec:caseII}) will consider a modification to $\lambda_n$ to allow the GA approach to construct networks with more or less intended cost.
...
\begin{table}[htp]
\centering
\caption[Demand Satisfaction Results for Various sDEP-Constructed VWNs for Preliminary Simulations]{Demand Satisfaction Results for Various sDEP-Constructed VWNs}
\begin{tabular}{|c|c|} 
\hline
...
\begin{figure}[htp]
	\centering
	\includegraphics[height=0.45\textheight]{Figures/Prelim_GAEval_VAE_HistogramBoxplot}
...
\begin{table}[htp]
\centering
\caption[Demand Satisfaction Results for Various GA-Constructed VWNs for Preliminary Simulations]{Demand Satisfaction Results for Various GA-Constructed VWNs}
...
Comparing the demand satisfaction for the two approaches shows the GA providing an average difference of 0.1\% demand satisfaction for the 13-, 14-, and 15-BS solutions.

\iffalse
Demand satisfaction approaches 100\% until reaching it for all sDEP solutions containing 12 or more BSs.  This covers the latter two ranges described while discussing VWN cost (i.e., $\alpha \approx \left[ \expnumber{2.2}{4},\, \expnumber{4.6}{4} \right]$ and $\alpha \geq \expnumber{1.0}{5}$), which generate 100\% demand satisfaction for the scenarios that they were optimized for.  That is, VWNs constructed by the sDEP that have similar (or higher) VWN cost to the average GA solution have perfect demand satisfaction.  Similarly, 48 of the 50 GA approaches also have 100\% demand satisfaction.  These solutions are over allocating resources to the VWNs, but this shows that the GA does not allocate ineffectively.

Further, the second sDEP range (i.e., $\alpha \approx \left[ \expnumber{4.6}{2},\, \expnumber{1.0}{4} \right]$) also allocates sufficient resources (12 BSs) to provide 100\% demand satisfaction.  The single 12-BS GA solution, which shows the GA is capable of finding an equivalent cost network to the sDEP in this range, also has 100\% demand satisfaction.  For this critical range where the sDEP and the best GA findings intersect, the GA provides a 100\% demand satisfaction network for no additional cost than the sDEP, and provides it in 189.9 seconds, or a 36.4x CPU time speedup; for equal cost networks, the GA provides a network with zero error in these preliminary simulations.

The first sDEP range (i.e., $\alpha < \expnumber{4.6}{2}$) is wholly described by solutions that provide less than 100\% demand satisfaction, but quickly approach that value with obviously diminishing returns.  This does inform to appropriate values of $\alpha$ to consider within the VNB to aim for specific percent demand satisfactions, though this will likely change according problem parameters.  What this range most importantly shows is that there does not exist, at least as far as the sDEP is an approximation of the original two-stage stochastic optimization program, any solutions that give equal demand satisfaction for less cost than the GA's solution.  This shows that the GA's intent---providing a minimum cost network for maximal demand satisfaction with low error to the optimal---is validly approached; the GA optimizes for a network that provides 100\% demand satisfaction, and provides that optimization with zero error.
\fi

As mentioned within analysis of VWN cost, the two following test cases (\Cref{sec:caseI,sec:caseII}) consider a modification to $\lambda_n$ that allows for the GA to consider networks not aiming to provide 100\% demand satisfaction.

Each sDEP solution is an optimization for the specific scenarios in $\hat{\Omega}$ that were used to generate the solution.  However, when new scenarios arise that are not in $\hat{\Omega}$, it is expected for those solutions to no longer perform as optimally.  \Cref{fig:Prelim_VWNCompSatisEval} visualizes the proportional demand satisfaction for both the sDEP and GA approaches when sliced to a new set of 50 scenarios as evaluated by the adaptive slicing model of \Cref{subsec:dep_slicing}.  The new scenarios are independently generated using the same method as the original scenarios, but with 200 demand points per scenario, each with 66.7 kbps rate demand.

\begin{figure}[ht]
	\centering
	\includegraphics[height=0.45\textheight]{Figures/Prelim_EvalO25_ComparisonSatisfactionFull}
	\caption[Demand satisfaction of the preliminary simulations evaluated against 50 new scenarios]{Proportional demand satisfaction of the sDEP and GA approaches sliced to 50 new scenarios using the adaptive slicing model.}
	\label{fig:Prelim_VWNCompSatisEval}
\end{figure}

For the range of $\alpha \leq 100$, the sDEP solutions perform slightly better for the new scenario sets compared to the scenarios they were optimized for.  For these larger scenarios which distribute demand more thoroughly, the sDEP solutions are capable of marginally higher demand satisfaction without the unneeded expense of the GA constructed VWN.  However, for all $\alpha > 100$, these new scenarios cause demand satisfaction to slightly decrease; a closer look over this range is shown in %\Cref{}.
